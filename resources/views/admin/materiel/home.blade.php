                ...
                <div class="p-6 text-gray-900">
                    ...
                    @if (auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'technician')
                        <x-primary-button class="mb-5">
                            <a href="materiels/create">ajouter un nouveau matériel</a>
                        </x-primary-button>
                    @endif
                        ...
                        @else
                            @foreach ($materiels as $materiel)
                                <div class="max-w-sm rounded overflow-hidden shadow-lg flex flex-col">
                                    @if ($materiel->image)
                                        <img class="w-full h-[200px] object-cover border border-gray-300"
                                            src="data:image/jpeg;base64,{{ base64_encode($materiel->image) }}"
                                            alt="Materiel Image">
                                    @else
                                        <div
                                            class="w-full h-[200px] flex items-center justify-center border border-gray-300">
                                    ...
                                    <div class="flex flex-col justify-between flex-grow">
                                        ...
                                        <div class="px-6 pb-4 mt-auto">
                                            <a href="{{ route('materiels.view', ['id' => $materiel->id]) }}"
                                                class="inline-block bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded">Voir
                                                plus</a>
